...
function handleRequest(router, req, res) {
  ...
  if (parsed.pathname === '/shardKeys' && req.method === 'PUT') {
    return addShardKey(router, req, res);
  }

  if (parsed.pathname === '/shardKeys' && req.method === 'GET') {
    return getShardKeys(router, req, res);
  }
...
function getShardKeys(router, req, res) {
  var keys = router.getShardKeys();
  res.end(JSON.stringify({shardKeys: keys}));
}
...
function addNode(router, req, res) {
  getBody(req, function(err, body) {
    ...
    var name = body.name;
    var type = body.type;
    router.addNode(name, type, function(err, nodes) {
      if (err) {
        res.statusCode = 500;
        return res.end(err);
      }
